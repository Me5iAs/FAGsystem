...
export class GInputComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data:inputI) {
      ...
      data.SoloLectura = data.SoloLectura || false;
      // console.log(data);
  ...
  onTextoL(elem, event){
    ...
    this.InputForm.controls[elem["Nombre"]].setValue(event.target.innerHTML);
  }
  ...
  public previewImage(event) {
    ...
    // console.log(file);
    let existe = file.type.indexOf("image");
    if (existe == -1) {
      return;
    }
